...
fn app() -> Element {
    ...
    let mut namespace = use_signal(|| "my::namespace".to_string());
    let mut data = use_signal(|| include_str!("../../data/test.svc").to_string());
    let mut rhai_code =
        use_signal(|| include_str!("../../example-generators/cpp-like.rhai").to_string());
    let mut type_mappings =
        use_signal(|| include_str!("../../example-generators/cpp-like.tym").to_string());
                ...
                section { id: "playground", class: "playground",
                    ...
                    form {
                        ...
                        input {
                            r#type: "text",
                            value: "{namespace}",
                            oninput: move |e| namespace.set(e.value().clone()),
                        }
                        ...
                        textarea {
                            rows: "25",
                            value: "{rhai_code}",
                            oninput: move |e| rhai_code.set(e.value().clone()),
                        }
                        ...
                        textarea {
                            rows: "5",
                            value: "{type_mappings}",
                            oninput: move |e| type_mappings.set(e.value().clone()),
                        }

                        button {
                            onclick: move |e| {
                                e.prevent_default();
                                let data = data.read().trim().to_owned();
                                let namespace = namespace.read().trim().to_owned();
                                let typemap = type_mappings.read().trim().to_owned();
                                let rhai = rhai_code.read().trim().to_owned();
                                match ssd_core::generate_web(HashMap::default(), &namespace, &rhai, &typemap, &data, *debug_mode.read()) {
                        ...
                        div { id: "output", style: if result.read().is_none() { "display: none;" } else { "" }, pre { "{result.read().clone().unwrap_or_default()}" } }
                    }
                }
            }
        )
